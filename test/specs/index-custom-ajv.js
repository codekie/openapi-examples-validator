...
const path = require('path'),
    ...
    { getValidatorFactory } = require('../../src/validator'),
    {
        loadTestData,
        getPathOfTestData
...
function customFactory(specSchema, ignoreFormats) {
    ...
    describe('validateExamples', function() {
        describe('API version 2', function() {
            it('should successfully validate the file', async function() {
                (await validateExamples(loadTestData('v2/valid-single-example'),
                    { validatorFactory: customFactory })).valid.should.equal(true);
            });
        });
    });
        ...
        describe('API version 2', function() {
            it('should successfully validate the file', async function() {
                (await validateExample(
                    ...
                    { validatorFactory: customFactory }
        ...
        describe('API version 2', function() {
            it('should successfully validate the file', async function() {
                const result = await validateExamplesByMap(FILE_PATH__EXTERNAL_EXAMPLES_SCHEMA,
                    ...
                    { validatorFactory: customFactory, cwdToMappingFile: true });
    ...
    describe('validateFile', function() {
        ...
        describe('collect statistics', () => {
            it('with examples with missing schemas', async() => {
                structuredClone(await validateFile(getPathOfTestData('v2/simple-example'),
                    { validatorFactory: customFactory })).statistics.should.deep
                    .equal({
            ...
            it('without examples', async() => {
                structuredClone(await validateFile(getPathOfTestData('v2/valid-without-examples'),
                    { validatorFactory: customFactory })).statistics
                    .should.deep.equal({
                        schemasWithExamples: 1,
            ...
            it('without schema', async() => {
                structuredClone(await validateFile(getPathOfTestData('v2/valid-without-schema'),
                    { validatorFactory: customFactory })).statistics
                    .should.deep.equal({
                        schemasWithExamples: 1,
